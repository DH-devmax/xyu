...
export const DHBrandLogo: React.FC<DHBrandLogoProps> = ({ size = 40, decorative = false, showLabel = false }) => (
  <Stack direction="row" spacing={1.25} sx={{ minWidth: 0, alignItems: 'center' }}>
    <Box component="img" src="/static/favicon.png" alt={decorative ? '' : 'DH闲不下来'} aria-hidden={decorative || undefined} sx={{ display: 'block', width: size, height: size, flexShrink: 0, objectFit: 'contain' }} />
    {showLabel && <Box sx={{ minWidth: 0 }}><Typography noWrap sx={{ fontSize: 15, fontWeight: 750 }}>DH闲不下来</Typography></Box>}
  </Stack>
);
