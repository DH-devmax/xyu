...
export const AudioMessage: React.FC<AudioMessageProps> = ({ messageKey, src, outgoing, initialDuration = 0 }) => {
  ...
  '&:focus-visible': {
    '--minimal-ring-opacity': '1',
    '--minimal-ring-color': 'rgb(var(--minimal-color-brand-400)/var(--minimal-ring-opacity,1))',
  },
}]}
      >
        {playbackState === 'loading' ? <MuiBox component={Loader2} sx={{ 'height': '1rem', 'width': '1rem', 'animation': 'spin 1s linear infinite' }} />
          : playbackState === 'playing' ? <MuiBox component={Pause} sx={{ 'height': '1rem', 'width': '1rem', 'fill': 'currentColor' }} />
            : playbackState === 'error' ? <MuiBox component={RotateCcw} sx={{ 'height': '1rem', 'width': '1rem' }} />
              : <MuiBox component={Play} sx={{ 'marginLeft': '1px', 'height': '1rem', 'width': '1rem', 'fill': 'currentColor' }} />}
      </MuiBox>
      <MuiBox component='div' sx={{ 'minWidth': '0', 'display': 'flex', 'flex': '1 1 0%', 'alignItems': 'center', 'gap': '.5rem' }}>
        <MuiBox component='div' sx={{ 'display': 'flex', 'height': '1.25rem', 'flex': '1 1 0%', 'alignItems': 'center', 'gap': '.125rem' }} aria-hidden="true">
          {waveformBars.map(/* barHeight 和 barIndex 分别决定稳定波形柱的像素高度与 React 键。 */ (barHeight, barIndex) => (
            <MuiBox component='span'
              ...
              sx={[{ 'width': '.125rem', 'borderRadius': '9999px' }, playbackState === 'playing' ? { 'animation': 'pulse 2s cubic-bezier(.4,0,.6,1) infinite' } : {}, outgoing ? {
  '--minimal-bg-opacity': '1',
  'backgroundColor': 'rgb(var(--minimal-color-brand-100)/var(--minimal-bg-opacity,1))',
} : playbackState === 'error' ? {
  '--minimal-bg-opacity': '1',
  ...
  '--minimal-bg-opacity': '1',
  'backgroundColor': 'rgb(var(--minimal-color-brand-400)/var(--minimal-bg-opacity,1))',
}]}
  ...
  '--minimal-text-opacity': '1',
  'color': 'rgb(var(--minimal-color-brand-100)/var(--minimal-text-opacity,1))',
} : playbackState === 'error' ? {
  '--minimal-text-opacity': '1',
