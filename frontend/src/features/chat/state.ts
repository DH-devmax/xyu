...
// UnreadBadgeSize 描述未读徽标的最小宽度和横向内边距。
export interface UnreadBadgeSize {
  // width 固定单数字徽标宽度，多数字徽标由内容撑开。
  width?: number;
  // minWidth 保证多数字徽标不会小于单数字直径。
  minWidth: number;
  // paddingInline 为双数字和 99+ 提供额外横向空间。
  paddingInline?: number;
}

/** 根据未读文本长度返回稳定徽标尺寸，单数字必须为正圆，双数字及 99+ 才可横向扩展。 */
export const unreadBadgeSize = (count: number): UnreadBadgeSize => {
  // normalized 保存用于区分单数字与多数字徽标的规范未读数量。
  const normalized = Number.isFinite(count) ? Math.max(0, Math.floor(count)) : 0;
  return normalized < 10
    ? { width: 20, minWidth: 20 }
    : { minWidth: 20, paddingInline: 6 };
};
