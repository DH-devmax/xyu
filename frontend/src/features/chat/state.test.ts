...
import { collectChatReadReceipts,filterChatSessions,formatClock,isChatAbortError,isCurrentChatRequest,markOutgoingMessagesReadByIncoming,mergeLiveMessage,mergeOlderMessages,messageTime,unreadBadgeLabel,unreadBadgeSize } from './state';
...
test('Chat 未读徽标为单数字保留正圆尺寸，多数字才横向扩展',
  ...
  () => {
    ...
    expect(unreadBadgeSize(1)).toEqual({ width: 20, minWidth: 20 });
    expect(unreadBadgeSize(10)).toEqual({ minWidth: 20, paddingInline: 6 });
    expect(unreadBadgeSize(99)).toEqual({ minWidth: 20, paddingInline: 6 });
  });
