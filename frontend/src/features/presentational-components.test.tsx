...
describe('前端纯展示组件', /* 当前回调处理无浏览器依赖的展示组件断言。 */ () => {
  test('卡密图标覆盖各交付类型', /* 当前回调处理库存图标类型分支。 */ () => {
    expect(render(<CardIcon type="text" />)).toContain('lucide-file-text');
    expect(render(<CardIcon type="image" />)).toContain('lucide-image');
    expect(render(<CardIcon type="api" />)).toContain('lucide-code');
    expect(render(<CardIcon type={'unknown' as never} />)).toContain('lucide-credit-card');
  });
  ...
  test('批量阶段指示器只高亮当前阶段', /* 当前回调处理批量发布步骤样式分支。 */ () => {
    ...
    expect(markup).toContain('data-phase="preview" data-active="true"');
  ...
  test('通知事件选择器渲染已选与未选事件', /* 当前回调处理通知事件列表的选择状态。 */ () => {
    ...
    expect(selected).toContain('data-selected="false"');
  });
