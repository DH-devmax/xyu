...
describe('ItemList batch state',
  ...
  test('covers all batch status labels and style groups',
    ...
    expect(batchStatusTone('success')).toBe('success');
    expect(batchStatusTone('completed')).toBe('success');
    expect(batchStatusTone('partially_failed')).toBe('error');
    expect(batchStatusTone('failed')).toBe('error');
    expect(batchStatusTone('running')).toBe('info');
    expect(batchStatusTone('canceled')).toBe('default');
    expect(batchStatusTone('pending')).toBe('warning');
    });
});
