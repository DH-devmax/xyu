...
func Migrate(ctx context.Context, db *sql.DB, dialect Dialect) error {
	...
	// 旧上游版本使用 00038-00041；本地 00038 已由 brain_runtime 占用，先接管完整上游结构以避免重复加列。
	if err := adoptUpstreamV107History(ctx, db, dialect); err != nil {
		return fmt.Errorf("兼容上游迁移历史: %w", err)
	}
	if // err 用于本次流程后续判断的err
