...
func TestOpenAPILocalResourceMutationResponses(t *testing.T) {
	...
	// deliveryTemplateCreateRecorder 保存包含订单和卡密变量占位符的模板创建响应。
	...
	// deliveryTemplateID 是后续模板读取、规则绑定、更新和删除操作使用的路径标识。
	...
	// templateRuleRecorder 保存带模板变量绑定的真实规则创建响应。
	...
	// templateRuleResponse 保存模板规则创建响应主键，供列表和删除成功场景复用。
	var templateRuleResponse mutationIDResponse
	// decodeErr 表示解析模板规则创建响应失败。
	...
	// templateRulesRecorder 保存模板变量绑定字段的真实规则列表响应。
	...
	serveOpenAPISuccess(t, handler, finalCookie, http.MethodDelete, "/api/v1/automation-rules/"+strconv.FormatInt(templateRuleResponse.ID, 10), "")
